      ...
      <section class="writer-panel">
        ...
        <form [formGroup]="form" (ngSubmit)="submit()" class="story-form">
          ...
          <label>
            ...
            <span
              class="field-hint"
              [class.over-limit]="wordCount() > maxWords"
            >
              {{ wordCount() }} / {{ maxWords }} words
            </span>
            @if (quoteMissing()) {
              <span class="field-hint over-limit"
                >Today's quote is missing from your story.</span
              >
            }
          </label>
          ...
          @if (!isAuthenticated()) {
            <p class="muted">
              ...
              <a routerLink="/register">register</a> to share your story.
            </p>
          }

          <button
            type="submit"
            [disabled]="form.invalid || submitting() || loading() || !quote()"
          >
...
export class DashboardPageComponent {
  ...
  readonly form = this.formBuilder.group({
    ...
    content: [
      ...
      [
        ...
        wordLimitValidator(this.maxWords),
      ],
    ],
  });
  ...
  quoteMissing(): boolean {
    ...
    return Boolean(
      quote && content.trim() && !includesQuote(content, quote.text),
    );
  }

  private applyQuoteRequirement(quote: QuoteResponse): void {
    this.quote.set(quote);
    this.form.controls.content.addValidators(
      quoteInclusionValidator(quote.text),
    );
...
function quoteInclusionValidator(quoteText: string): ValidatorFn {
  return (control: AbstractControl): ValidationErrors | null => {
    return includesQuote(control.value, quoteText)
      ? null
      : { quoteMissing: true };
  };
}
...
function includesQuote(
  content: string | null | undefined,
  quoteText: string,
): boolean {
  return normalizeForQuoteMatch(content).includes(
    normalizeForQuoteMatch(quoteText),
  );
}
